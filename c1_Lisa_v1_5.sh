...
mkdir -p "$output_dir/crontab_file"
...
print_progress_bar() {
    local progress=(${1})
    local total=(${2})
    local filledLength=$(((${progress} * 50) / ${total}))
    local unfilledLength=$((50 - ${filledLength}))
    local progressBar=$(printf "%-${filledLength}s" "#" | sed 's/ /#/g')
    local unfilled=$(printf "%-${unfilledLength}s" " ")

    echo -ne "\rProgress: [${progressBar}${unfilled}] ${progress}%"
}
...
# write_output "cat $output_dir/$art_dir/file_time_stamp.txt | grep -E 'tar.gz\$' " "file_time_stamp_tar_gz.txt" "System"
# write_output "cat $output_dir/$art_dir/file_time_stamp.txt | grep -E '.tgz\$'  " "file_time_stamp_tgz.txt" "System"
# write_output "cat $output_dir/$art_dir/file_time_stamp.txt | grep -E '.zip\$'  " "file_time_stamp_zip.txt" "System"
...
## Check
write_output "ls -alR /tmp /dev/shm /var/tm" "ls_tmp_dev_var.txt" "System"
...
for command in "${commands[@]}"; do
    write_output "$command" "losf.txt" "System"
done
...
## Check_chkproc
write_output "chkproc" "chkproc.txt" "Process"
...
for user_home in /home/* /root; do
    ...
    if [ -f "$user_home/.zsh_history" ]; then
        ...
        echo "No .zsh_history for $username" >> "$output_dir/Users/zsh_command_history_$username.txt"
        write_output "cat $user_home/.zsh_history" "zsh_command_history_$username.txt" "Users"
    fi
...
for log_pattern in "${logs[@]}"; do
    for log in /var/log/$log_pattern; do
        ...
        if [ -f "$log" ]; then
            ...
            echo "No Logfile for $logname" >> "$output_dir/Logs/$logname.txt"
            write_output "cat $log" "$logname.txt" "Logs"
        fi
    done
done
...
for log in /var/spool/cron/crontabs/*; do
    logname=$(basename "$log")
    if [ -f "$log" ]; then
        write_output "cat $log" "$logname.txt" "crontab_file"
    else
        echo "No Logfile for $logname" >> "$output_dir/Logs/$logname.txt"
        write_output "cat $log" "$logname.txt" "crontab_file"
    fi
...
# 메인 스크립트
totalSteps=100
for ((step=1; step<=totalSteps; step++)); do
    print_progress_bar ${step} ${totalSteps}
    sleep 0.1 
done
