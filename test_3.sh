...
if [ -n "$usb_mount_path" ]; then
    cp -r "$usb_mount_path" "$output_dir/usbmount"
    echo "Check the USB mount" >> "$output_dir/usbmount/USB_Mount.txt"
else
    echo "USB mount not found" >> "$output_dir/usbmount/USB_Mount.txt"
fil
...
## /var/log
write_output "cat /etc/rsyncd.conf" "var_log_rsyncd" "Logs"
write_output "cat /var/log/messages* | grep -i "rsync"" "var_log_message" "Logs"
write_output "cat /var/log/secure* | grep -i accepte" "var_log_secure_accepte" "Logs"
write_output "cat /var/log/secure* | grep -i fail" "var_log_secure_fail" "Logs"

## check syslog
write_output "cat /etc/syslog.conf" "syslog_conf.txt" "Logs"
...
write_output "cat /var/spool/cron/*" "cronfile.txt" "Logs"
